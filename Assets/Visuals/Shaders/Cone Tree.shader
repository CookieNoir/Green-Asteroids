Shader "Unlit/Cone Tree"
{
    Properties
    {
        _MainTex("Normal Color (1D)", 2D) = "white" {}
        _SecondTex("UV Color (1D)", 2D) = "white" {}
        _NormalColorPower("Normal Color Power", float) = 1.0
        _UVColorPower("UV Light Power", float) = 1.0
        _MixPower("Mix Power", float) = 1.0
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                fixed4 frag(v2f i) : SV_Target
                {
                    ...
                    float mixValue = pow(i.uv.y, _MixPower);
                    fixed4 col = ((1.0 - mixValue) * tex2D(_MainTex, float2(normalColor, 0.5))
                        + mixValue * tex2D(_SecondTex, float2(pow(i.uv.y, _UVColorPower), 0.5)))
